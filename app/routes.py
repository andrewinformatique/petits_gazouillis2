...
def enregistrer():
    ...
    if formulaire.validate_on_submit():
        ...
        db.session.commit()
        flash('Vous êtes maintenant enregistrés')
...
def editer_profil():
    ...
    if formulaire.validate_on_submit():
        ...
        db.session.commit()
        flash('Vos modifications ont été sauvrgardées.')
    ...
    return render_template('editer_profil.html', title = 'Éditer Profil', formulaire = formulaire)
